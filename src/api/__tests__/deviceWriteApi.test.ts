...
import { insertAttachment, insertAttachmentMetadata, insertEntityIndex, insertMemoryItem, insertMessage, linkMessageAttachment } from '../deviceWriteApi';
    ...
    describe('insertAttachmentMetadata', () => {
        it('inserts metadata with JSON payload', async () => {
            ...
            expect(mockDb?.runAsync).toHaveBeenCalledWith(
                ...
                expect.arrayContaining(['meta-1', 'att-1', 'gemini-2.0', 'transcript'])
            );
        });
    });
