...
describe("DeviceReadAPI", () => {
  ...
  describe("searchMemory", () => {
    it("uses FTS query for text filter", async () => {
      ...
      const firstSql = (mockDb?.getAllAsync as jest.Mock).mock
        .calls[0][0] as string;
    ...
    it("queries both memory and attachment_metadata sources", async () => {
      ...
      expect((mockDb?.getAllAsync as jest.Mock).mock.calls[0][0]).toContain(
        "FROM memory_items",
      );
      expect((mockDb?.getAllAsync as jest.Mock).mock.calls[1][0]).toContain(
  ...
  describe("reminders", () => {
    ...
    it("listReminders filters by min_due_at if provided", async () => {
      await listReminders({ min_due_at: 5000, limit: 10 });
