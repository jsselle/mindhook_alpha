...
export const searchMemory = async (args: {
    ...
    if (normalizedText) {
        ...
        const ftsMemoryRows = await db.getAllAsync<{ source_id: string; fts_rank: number }>(
        ...
        const ftsMetadataRows = await db.getAllAsync<{ source_id: string; fts_rank: number }>(
            `SELECT source_id, bm25(memory_search_fts) as fts_rank
             FROM memory_search_fts
    ...
    if (ftsRankByMemoryId.size > 0) {
        const placeholders = Array.from(ftsRankByMemoryId.keys()).map(() => '?').join(', ');
    ...
    const memoryWhere = memoryConditions.length > 0 ? `WHERE ${memoryConditions.join(' AND ')}` : '';
    ...
    if (ftsRankByMetadataId.size > 0) {
        const placeholders = Array.from(ftsRankByMetadataId.keys()).map(() => '?').join(', ');
    ...
    const metadataWhere = metadataConditions.length > 0 ? `WHERE ${metadataConditions.join(' AND ')}` : '';
