...
export const sortReminderRowsForDrawer = (
  rows: ReminderRow[],
): ReminderRow[] => {
...
export const useReminderDrawer = () => {
  ...
  const editReminderDate = useCallback(
    async (args: { reminder: ReminderRow; dueAt: number }) => {
      if (args.dueAt <= nowMs()) {
      ...
      try {
        await executeToolCall("update_reminder", {
          ...
          due_at: args.dueAt,
          ...(args.reminder.status === "triggered"
            ? { status: "scheduled" }
            : {}),
